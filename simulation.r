...
gen_adjacency_matrix <- function(size = 6, density = 1) {
    mat <- matrix(0, size + 2, size + 2)
    while (any(rowSums(mat[3:(size + 1), ]) == 0) || any(colSums(mat[, 2:size]) == 0)) {
        mat <- matrix(0, size + 2, size + 2)
        for (i in 2:(size + 1)) {
            for (j in 2:i) {
                if (i != j && runif(1) < density) {
...
gen_loading_matrix <- function(mat,
    ...
    } else if (length(m) == 2 && func == "uniform") {
        mat[2:(ncol(mat) - 1), 2:(ncol(mat) - 1)] <- runif((ncol(mat) - 2)^2, m[1], m[2]) * mat[2:(ncol(mat) - 1), 2:(ncol(mat) - 1)]
    }
    if (length(y_prime) == 1) {
...
gen_covariance <- function(beta, r = 1.) {
    I <- diag(nrow(beta))

    evec <- diag(beta %*% t(beta))

    psi <- diag(nrow(beta)) - diag(evec)
...
# working example
con_mat <- gen_adjacency_matrix(6, density = .8)
beta <- gen_loading_matrix(con_mat, x_prime = .6, m = c(.2, .8), y_prime = .6, func = "uniform")
cov_mat <- gen_covariance(beta)

eigen(cov_mat) ## if eigenvalues > 0 -> positive definite => it is a valid covariance matrix



# >>>> original Rhemtulla code
